"""
Business: Отправляет заявки с формы записи в Telegram через бота
Args: event - dict с httpMethod, body (JSON с name, phone, direction)
...
def handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if not name or not phone or not direction:
        return {
            'statusCode': 400,
            'headers': {
                'Content-Type': 'application/json',
    ...
    message = f"""🎭 Новая заявка с сайта Ритмы Гор
